...
public:
	...
	double rayTrace(double origin[3], double unit[3], ball balls[], int n, double altitute, double coeff, int limit){
		...
		if(index==-1){
			if(unit[2]>0) return 0; //ray hit the sky
				...
				if((int)(floor(tx)+floor(ty))%2==0) return (1-coeff)*color+coeff*rayTrace(origin2,unit2,balls,n,altitute,coeff,limit-1);
				else return 0;
			}
		}

		if(unit[2]<0&&distance>distanceToPlane){ //ray hit the ground
			double
			tx=origin[0]+distanceToPlane*unit[0],
			ty=origin[1]+distanceToPlane*unit[1];
...
int main(){
	...
	system("clear");
    ifstream text("RayTracing_GameOfLife.txt");
    string line;
	...
	while(1){
		...
		for(int i=0;i<HEIGHT/dH;i++) for(int j=0;j<WIDTH/dW;j++) platno[i][j]=0; 
		...
		for(int i=0;i<HEIGHT/dH;i++){
			cout<< "    ";
			for(int j=0;j<WIDTH/dW;j++) printf("%c",platno[i][j]);
			printf("\n");
		}
		cout<< "    Press ctr+c to exit to menu\n";
